...
Adds Parallel AI MCP integration to NeoPaw for advanced web research capabilities.
...
- **Non-blocking Design** - Uses NeoPaw scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. NeoPaw already set up and running
...
Add Parallel AI MCP servers after the neopaw server:
```typescript
const mcpServers: Record<string, any> = {
  neopaw: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__neopaw__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__neopaw__send_message
   4. Use mcp__neopaw__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo neopaw-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep neopaw  # macOS
...
- Check task runs: `tail -f logs/neopaw.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.neopaw` (macOS) or `systemctl --user restart neopaw` (Linux)
