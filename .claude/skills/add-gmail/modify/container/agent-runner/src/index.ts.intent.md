...
- Added: `gmail` MCP server alongside the existing `neopaw` server:
  ```
  gmail: {
    command: 'npx',
...
## Invariants
- The `neopaw` MCP server configuration is unchanged
...
## Must-keep
- The `neopaw` MCP server with its environment variables
