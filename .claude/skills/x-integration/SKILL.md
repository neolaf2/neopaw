---
name: x-integration
description: X (Twitter) integration for NeoPaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** NeoPaw v1.0.0. Directory structure may change in future versions.
...
1. **NeoPaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep neopaw (macOS) or systemctl --user status neopaw (Linux)
```
...
| `NEOPAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/neopaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into NeoPaw, make the following modifications:
...
All paths below are relative to project root (`NEOPAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep neopaw  # macOS — should show PID and exit code 0 or -
# Linux: systemctl --user status neopaw
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/neopaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/neopaw.log | tail -20
```
...
docker run neopaw-agent ls -la /app/src/skills/
```
