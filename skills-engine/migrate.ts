...
import { BASE_DIR, CUSTOM_DIR, NEOPAW_DIR } from './constants.js';
import { initNeopawDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initNeopawDir();
  console.log('Skills system initialized. .neopaw/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .neopaw/custom/migration.patch',
      );
    } else {
