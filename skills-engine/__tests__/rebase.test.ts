...
describe('rebase', () => {
  ...
  it('rebase with one skill: patch created, state updated, rebased_at set', async () => {
    // Set up base file
    const baseDir = path.join(tmpDir, '.neopaw', 'base', 'src');
    ...
    const patchPath = path.join(tmpDir, '.neopaw', 'combined.patch');
    ...
    const stateContent = fs.readFileSync(
      path.join(tmpDir, '.neopaw', 'state.yaml'),
      'utf-8',
    );
  ...
  it('rebase flattens: base updated to match working tree', async () => {
    ...
    const baseContent = fs.readFileSync(
      path.join(tmpDir, '.neopaw', 'base', 'src', 'index.ts'),
      'utf-8',
    );
  ...
  it('rebase with multiple skills + custom mods: all collapsed into single patch', async () => {
    // Set up base files
    const baseDir = path.join(tmpDir, '.neopaw', 'base');
      ...
      custom_modifications: [
        {
          ...
          patch_file: '.neopaw/custom/001-tweaked-config.patch',
        },
      ],
    });
    ...
    const patchContent = fs.readFileSync(
      path.join(tmpDir, '.neopaw', 'combined.patch'),
      'utf-8',
    );
    ...
    const baseConfig = fs.readFileSync(
      path.join(tmpDir, '.neopaw', 'base', 'src', 'config.ts'),
      'utf-8',
    );
