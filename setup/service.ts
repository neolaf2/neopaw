...
<dict>
    ...
    <string>${projectRoot}/logs/neopaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/neopaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.neopaw');
  } catch {
...
/**
 * Kill any orphaned neopaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned neopaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/neopaw.service';
    ...
    unitPath = path.join(unitDir, 'neopaw.service');
    systemctlPrefix = 'systemctl --user';
  }

  const unit = `[Unit]
Description=NeoPaw Personal Assistant
...
StandardOutput=append:${projectRoot}/logs/neopaw.log
StandardError=append:${projectRoot}/logs/neopaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned neopaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable neopaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start neopaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active neopaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-neopaw.sh');
  const pidFile = path.join(projectRoot, 'neopaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-neopaw.sh — Start NeoPaw without systemd',
    ...
    '    echo "Stopping existing NeoPaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting NeoPaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/neopaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/neopaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NeoPaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/neopaw.log"`,
  ];
