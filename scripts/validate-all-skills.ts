 ...
 *   2. Initialize .neopaw/ (snapshot current source as base)
...
function resetWorkingTree(): void {
  ...
  if (fs.existsSync('.neopaw')) {
    fs.rmSync('.neopaw', { recursive: true, force: true });
  }
}

function initNeopaw(): void {
  execSync(
    'npx tsx -e "import { initNeopawDir } from \'./skills-engine/index\'; initNeopawDir();"',
