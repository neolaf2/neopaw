...
│  │    • mcp__neopaw__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── neopaw.log               # Host stdout
│   └── neopaw.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.neopaw.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'neopaw-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your neopaw installation
...
NeoPaw uses a hierarchical memory system based on CLAUDE.md files.
...
NeoPaw has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__neopaw__schedule_task]
        {
...
The `neopaw` MCP server is created dynamically per agent call with the current group's context.
...
NeoPaw runs as a single macOS launchd service.

### Startup Sequence

When NeoPaw starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned NeoPaw containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/neopaw.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/neopaw.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.neopaw.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep neopaw` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; NeoPaw auto-starts container runtime but may fail |
...
- `logs/neopaw.log` - stdout
- `logs/neopaw.error.log` - stderr
